/**
 * A const value for the device ID search parameter in device routes.
 */
export const DEVICE_ID_SEARCH_PARAM = 'deviceid';

/**
 * A const value for the routine ID search parameter in routine routes.
 */
export const ROUTINE_ID_SEARCH_PARAM = 'routineid';

/**
 * A const value for the URL for creating a routine.
 */
export const ADD_ROUTINE_URL = '/createroutine';

/**
 * A const value for the URL for editing a routine.
 */
export const EDIT_ROUTINE_URL = '/editroutine';

/**
 * A const value for the URL for viewing a routine.
 */
 export const VIEW_ROUTINE_URL = '/routine';

/**
 * A const value for the URL for a user to log in.
 */
export const LOGIN_URL = '/login';

/**
 * A const value for the URL for a user to log out.
 */
export const LOGOUT_URL = '/logout';

/**
 * A const value for the URL for a user to sign up.
 */
export const SIGNUP_URL = '/signup';

/**
 * A const value for the URL for the user account page.
 */
export const ACCOUNT_URL = '/account';

/**
 * A const value for the URL for a user's routines list.
 */
export const ROUTINES_URL = '/routines';

/**
 * A const value for the URL for a user's devices list.
 */
export const DEVICES_URL = '/devices';

/**
 * A const value for the URL for a user's devices list.
 */
export const ADD_DEVICE_URL = '/createdevice';

/**
 * A const value for the URL for editing a device.
 */
export const EDIT_DEVICE_URL = '/editdevice';

/**
 * A const value for the URL for adding a device to a routine.
 */
export const ADD_DEVICE_TO_ROUTINE_URL = '/addtoroutine';
